...
export default function POSPage() {
  ...
  const fetchOrders = useCallback(() => {
    fetch(`/api/orders?department=${dept}`)
      .then((r) => r.json())
      .then((data: Order[]) => {
            ...
            {(["breakfast", "lunch"] as const).map((d) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
                  dept === d ? "bg-white text-stone-900" : "text-stone-400 hover:text-white"
                }`}
              >
                {d === "breakfast" ? "早餐部" : "午餐部"}
              </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-stone-800 rounded-xl p-0.5">
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
                view === "order" ? "bg-white text-stone-900" : "text-stone-400 hover:text-white"
              }`}
            >
              點餐
            </button>
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all relative ${
                view === "queue" ? "bg-white text-stone-900" : "text-stone-400 hover:text-white"
              }`}
            >
              訂單
              {pendingCount > 0 && (
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-80 bg-stone-900 border-l border-stone-800 flex flex-col shrink-0">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {cart.length === 0 && (
                <p className="text-stone-600 text-center mt-10 text-sm">點擊菜單加入品項</p>
              )}
              {cart.map((c) => (
                <div key={c.item.id} className="bg-stone-800 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-7 h-7 rounded-lg bg-stone-700 flex items-center justify-center text-sm hover:bg-stone-600"
                    >
                      −
                    </button>
            ...
            <div className="p-4 border-t border-stone-800 space-y-3">
              ...
              {cart.length > 0 && (
                <button
                  onClick={() => setCart([])}
                  className="w-full text-stone-500 hover:text-red-400 text-sm py-1 transition-colors"
                >
                  清空
                </button>
              )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {activeOrders.length === 0 && (
              <p className="text-stone-600 text-center col-span-full mt-20">目前沒有訂單</p>
            )}
                  ...
                  <div className="flex gap-2">
                    ...
                    {order.status !== "cancelled" && (
                      <button
                        ...
                        className="px-4 py-2 rounded-xl text-sm text-red-400 bg-red-500/10 hover:bg-red-500/20 transition-all"
                      >
                        取消
                      </button>
                    )}
